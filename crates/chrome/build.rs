...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if target_os == "windows" {
        #[cfg(windows)]
        {
            ...
            res.set_manifest_file("src/platform/windows/servoshell.exe.manifest");
            res.compile().unwrap();
        }
        #[cfg(not(windows))]
        panic!("Cross-compiling to windows is currently not supported");
    } else if target_os == "macos" {
        println!("cargo:rerun-if-changed=src/platform/macos/count_threads.c");
        cc::Build::new()
            .file("src/platform/macos/count_threads.c")
