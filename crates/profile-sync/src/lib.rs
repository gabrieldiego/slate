...
impl SettingsSyncCyclePolicy {
    ...
    pub fn check_selected_retention_provider_roles(
        &self,
        ineligible_selected: usize,
        health: &SettingsSyncHealthReport,
    ) -> Result<(), ProfileSyncPolicyError> {
        self.check_provider_maximum(
            "ineligible selected retention providers",
            0,
            ineligible_selected,
            health,
        )
...
impl<'a> BroadwebdSettingsSyncScheduler<'a> {
    ...
    pub fn try_plan_once_with_membership_log_and_stored_retention_providers(
        &self,
        database: &SlateProfileDatabase,
        config: &SettingsSyncSchedulerConfig,
        membership_log_root_id: &str,
        ...
        SettingsSyncStoredRetentionProviderMembershipPlanAttempt,
        ProfileSyncCycleWithHealthError,
    > {
        let membership_log_plan =
            self.plan_membership_log(database, config.profile.as_str(), membership_log_root_id)?;
        let cycle = match self.plan_once_with_stored_retention_providers(
            database,
            config,
            signer,
            max_stored_providers,
        ...
        Ok(SettingsSyncStoredRetentionProviderMembershipPlanAttempt {
            membership_log_publication: membership_log_plan.local_publication,
            membership_log_preview: membership_log_plan.remote_preview,
            cycle,
        })
    ...
    pub fn run_once_with_stored_retention_provider_handles(
        &self,
        database: &SlateProfileDatabase,
        config: &SettingsSyncSchedulerConfig,
        secrets: SettingsSyncRuntimeSecrets<'_>,
...
mod tests {
    ...
    #[test]
    fn membership_stored_scheduler_runs_with_protocol_materialized_provider_without_loopback() {
        let network = InProcessBroadwebNetwork::new();
        let device_state_root = test_state_root("membership-protocol-provider-device");
        let provider_state_root = test_state_root("membership-protocol-provider-provider");
        let db_root = test_state_root("membership-protocol-provider-db");
        let device_id = "membership-protocol-provider-device";
        let provider_id = "membership-protocol-materialized-provider";
        let provider_endpoint = "/dnsaddr/membership-provider.example.test/p2p/materialized";
        let device_daemon = network
            .daemon_for_device(&device_state_root, ResourceBudget::default(), device_id)
            .expect("start in-process membership protocol device daemon");
        let provider_daemon = network
            ...
            .expect("start socketless membership protocol provider daemon");
        let database = SlateProfileDatabase::open_resolved_with_device_id(
            db_root.join(DEFAULT_DATABASE_FILE_NAME),
            device_id,
        )
        .expect("open membership protocol scheduler database");
        let content_key = ProfileSyncContentKey::from_bytes([89; PROFILE_SYNC_CONTENT_KEY_BYTES]);
        let signer =
            ProfileSyncDeviceSigner::generate(device_id).expect("generate membership signer");
        let provider_signer =
            ProfileSyncDeviceSigner::generate(provider_id).expect("generate provider signer");
        register_test_content_key_epoch(&database, DEFAULT_PROFILE_ID);
        let enroll_device = ProfileSyncMembershipRecord {
            profile: DEFAULT_PROFILE_ID.to_string(),
            record_id: "epoch-1-enroll-membership-protocol-provider-device".to_string(),
            schema_version: PROFILE_SYNC_MEMBERSHIP_RECORD_SCHEMA_VERSION,
            membership_epoch: DEFAULT_PROFILE_SYNC_MEMBERSHIP_EPOCH,
            record_kind: PROFILE_SYNC_MEMBERSHIP_RECORD_KIND_ENROLL_DEVICE.to_string(),
        ...
        database
            ...
            .expect("bootstrap membership protocol scheduler device");
        let enroll_provider = ProfileSyncMembershipRecord {
            profile: DEFAULT_PROFILE_ID.to_string(),
            record_id: "epoch-2-enroll-membership-protocol-provider".to_string(),
            schema_version: PROFILE_SYNC_MEMBERSHIP_RECORD_SCHEMA_VERSION,
            membership_epoch: DEFAULT_PROFILE_SYNC_MEMBERSHIP_EPOCH + 1,
            record_kind: PROFILE_SYNC_MEMBERSHIP_RECORD_KIND_ENROLL_PROVIDER.to_string(),
            device_id: provider_id.to_string(),
        ...
        database
            ...
            .expect("enroll protocol materialized provider authority");
        database
            .upsert_storage_provider(&StorageProviderUpdate {
                ...
                ..test_storage_provider_update(
                    ...
                    provider_id,
                    "socketless-protocol-provider",
                    "Membership protocol pinner",
                    true,
                    true,
                    true,
                )
            })
            .expect("write membership protocol provider metadata");
        database
            .set_sync_setting_text(DEFAULT_PROFILE_ID, SYNC_DOMAIN_SETTINGS, "ui.theme", "teal")
            .expect("write membership protocol scheduler setting");
        ...
        let run = scheduler
            ...
            .expect("membership stored scheduler uses protocol materialized provider");
        ...
        assert_eq!(
            ...
            vec![super::SettingsSyncSelectedMultiaddrMaterializationRequest {
                provider_id: provider_id.to_string(),
                endpoint: slate_routing::Multiaddr::parse(provider_endpoint)
                    .expect("membership stored provider multiaddr"),
            }]
        );
        assert!(
            run.selected_protocol_materialization_plan()
    ...
    fn broadwebd_settings_sync_scheduler_runs_with_stored_retention_provider_handles() {
        ...
        let _unmaterialized_provider_daemon = network
            .daemon_for_availability_provider(
                &unmaterialized_state_root,
                ResourceBudget::default(),
                "runtime-scheduler-stored-run-extra",
            )
            .expect("start in-process unmaterialized stored provider daemon");
        let database = SlateProfileDatabase::open_resolved_with_device_id(
            db_root.join(DEFAULT_DATABASE_FILE_NAME),
            "runtime-scheduler-stored-run-a",
        )
        .expect("open scheduler stored-run settings database");
        let profile = "schedulerstoredrunprofile";
        let settings_root_id = "settings/latest";
        let content_key = ProfileSyncContentKey::from_bytes([82; PROFILE_SYNC_CONTENT_KEY_BYTES]);
        let signer = ProfileSyncDeviceSigner::generate("runtime-scheduler-stored-run-a")
            .expect("generate scheduler stored-run signer");
        register_test_content_key_epoch(&database, profile);
        database
            .register_sync_device_public_key(&SyncDevicePublicKeyRegistration {
                profile: profile.to_string(),
            ...
            .expect("register scheduler stored-run local trusted public key");
        database
            .set_sync_setting_text(profile, SYNC_DOMAIN_SETTINGS, "ui.theme", "teal")
    ...
    fn broadwebd_settings_sync_scheduler_runs_with_kubo_profile_sync_materialized_provider() {
        ...
        .expect("open Kubo scheduler settings database");
        let profile = "schedulerkuboproviderprofile";
        let settings_root_id = "settings/latest";
        let content_key = ProfileSyncContentKey::from_bytes([91; PROFILE_SYNC_CONTENT_KEY_BYTES]);
        let signer = ProfileSyncDeviceSigner::generate("runtime-scheduler-kubo-a")
            .expect("generate Kubo scheduler local signer");
        register_test_content_key_epoch(&database, profile);
        database
            .register_sync_device_public_key(&SyncDevicePublicKeyRegistration {
                profile: profile.to_string(),
            ...
            .expect("register Kubo scheduler local trusted public key");
        authorize_test_storage_provider(&database, profile, provider_id);
        database
            .upsert_storage_provider(&StorageProviderUpdate {
            ...
            .expect("write Kubo stored provider metadata");
        database
            .set_sync_setting_text(profile, SYNC_DOMAIN_SETTINGS, "ui.theme", "teal")
            .expect("write Kubo scheduler local setting");
    ...
    fn broadwebd_settings_sync_scheduler_classifies_ineligible_retention_provider_handles() {
        ...
        let no_availability_state_root = test_state_root("scheduler-ineligible-no-availability");
        let no_transfer_state_root = test_state_root("scheduler-ineligible-no-transfer");
        let db_root = test_state_root("scheduler-ineligible-db");
        let device_daemon = network
            .daemon_for_device(
                &device_state_root,
                ResourceBudget::default(),
                "runtime-scheduler-ineligible-a",
            )
            .expect("start in-process profile-sync scheduler device daemon");
        let no_availability_provider_id = "local-fixture-scheduler-no-availability";
        let no_availability_daemon = network
            .daemon_for_provider_with_roles(
                &no_availability_state_root,
                ResourceBudget::default(),
                no_availability_provider_id,
                "local-fixture-custom",
                BroadwebdProfileSyncProviderRoles {
                    availability: false,
                    mutable_roots: false,
            ...
            .expect("start in-process no-availability provider daemon");
        let no_transfer_provider_id = "local-fixture-scheduler-no-transfer";
        let no_transfer_daemon = network
            .daemon_for_provider_with_roles(
                &no_transfer_state_root,
                ResourceBudget::default(),
                no_transfer_provider_id,
            ...
            .expect("start in-process no-transfer provider daemon");
        let database = SlateProfileDatabase::open_resolved_with_device_id(
            db_root.join(DEFAULT_DATABASE_FILE_NAME),
            "runtime-scheduler-ineligible-a",
        )
        .expect("open scheduler local settings database");
        let profile = "schedulerineligibleprofile";
        let settings_root_id = "settings/latest";
        let content_key = ProfileSyncContentKey::from_bytes([74; PROFILE_SYNC_CONTENT_KEY_BYTES]);
        let signer = ProfileSyncDeviceSigner::generate("runtime-scheduler-ineligible-a")
            .expect("generate scheduler local device signer");
        register_test_content_key_epoch(&database, profile);
        database
            .register_sync_device_public_key(&SyncDevicePublicKeyRegistration {
        ...
        let retention_provider_handles = [
            SettingsSyncRetentionProviderHandle::new(
                no_availability_provider_id,
                &no_availability_daemon,
            ),
            SettingsSyncRetentionProviderHandle::new(no_transfer_provider_id, &no_transfer_daemon),
        ];
        let latest_revision_before_plan = database
            .latest_sync_revision(profile)
            .expect("read latest scheduler revision");
        ...
        let plan = scheduler
            ...
            .expect("scheduler plan classifies ineligible retention provider handles");
        ...
        assert_eq!(plan.offline_retention_provider_count(), 0);
        assert_eq!(
            plan.ineligible_retention_provider_ids,
            vec![
                no_availability_provider_id.to_string(),
                no_transfer_provider_id.to_string()
            ]
        );
        ...
        assert_eq!(
            database
                .latest_sync_revision(profile)
                .expect("read latest revision after ineligible plan"),
            latest_revision_before_plan
        );
        assert!(
            database
                .profile_sync_roots(profile)
                .expect("read roots after ineligible plan")
                .is_empty()
        );

        let run_error = scheduler
            .run_once_selecting_retention_providers(
                &database,
                &config,
                SettingsSyncRuntimeSecrets::new(&content_key, &signer),
            ...
            .expect_err("scheduler run should reject ineligible retention providers");
        let ProfileSyncCycleWithHealthError::Policy(
            ProfileSyncPolicyError::ProviderMaximumExceeded {
                provider_role,
                maximum,
            ...
            panic!("expected ineligible selected retention provider error, got {run_error:?}");
        };
        ...
        assert!(
            health
                ...
                .fresh_online_provider_ids
                .iter()
                .any(|provider_id| provider_id == no_availability_provider_id)
        );
        assert!(
            health
                .provider_health
                .fresh_online_provider_ids
                .iter()
                .any(|provider_id| provider_id == no_transfer_provider_id)
        );
        assert!(
            database
                .profile_sync_roots(profile)
                .expect("read roots after rejected ineligible run")
                .is_empty()
        );
        let retained = device_daemon
            .profile_sync(BroadwebdProfileSyncRequest::ListRetainedObjects(
                BroadwebdProfileSyncProfileRequest::new(profile),
            ))
            .expect("local device can list retained objects after rejected scheduler run");
        assert_eq!(
            retained,
            BroadwebdProfileSyncResponse::RetainedObjects {
                object_ids: Vec::new(),
        ...
        let _ = std::fs::remove_dir_all(device_state_root);
        let _ = std::fs::remove_dir_all(no_availability_state_root);
        let _ = std::fs::remove_dir_all(no_transfer_state_root);
        let _ = std::fs::remove_dir_all(db_root);
    }
