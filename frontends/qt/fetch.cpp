 ...
 */
static const char *slateqt_fetch_filetype(const char *unix_path)
{
	int l;
 ...
 */
static slateurl *slateqt_get_resource_url(const char *path)
{
	char buf[PATH_MAX];
	slateurl *url = NULL;
...
static slateerror
slateqt_get_resource_data(const char *resname,
...
static slateerror slateqt_release_resource_data(const uint8_t *data)
{
	free((uint8_t *)data);
	return SLATEERROR_OK;
}
...
static struct gui_fetch_table fetch_table = {
	.filetype = slateqt_fetch_filetype,

	.get_resource_url = slateqt_get_resource_url,
	.get_resource_data = slateqt_get_resource_data,
	.release_resource_data = slateqt_release_resource_data,
	.mimetype = NULL,
	.socket_open = NULL,
	.socket_close = NULL,
};

struct gui_fetch_table *slateqt_fetch_table = &fetch_table;
