...
void slategtk_entry_set_icon_from_pixbuf(GtkWidget *entry,
...
/* exported interface documented in gtk/compat.h */
void slategtk_entry_set_icon_from_icon_name(GtkWidget *entry,
					 GtkEntryIconPosition icon_pos,
					 const gchar *id)
{
...
/* exported interface documented in gtk/compat.h */
GtkWidget *slategtk_image_new_from_stock(const gchar *id, GtkIconSize size)
{
#ifdef SLATEGTK_USE_ICON_NAME
...
void slategtk_widget_override_background_color(GtkWidget *widget,
					    GtkStateFlags state,
					    uint16_t a,
					    uint16_t r,
					    uint16_t g,
...
static void slategtk_layout_set_adjustment_step_increment(GtkAdjustment *adj,
						       int value)
{
...
#endif
	slategtk_layout_set_adjustment_step_increment(adj, 8);
}
...
const PangoFontDescription* slategtk_style_context_get_font(GtkStyleContext *style,
							 GtkStateFlags state)
{
...
gulong slategtk_connect_draw_event(GtkWidget *widget,
				GCallback callback,
				gpointer g)
{
#if GTK_CHECK_VERSION(3,0,0)
...
void slategtk_widget_modify_font(GtkWidget *widget,
...
gboolean slategtk_show_uri(GdkScreen *screen,
			const gchar *uri,
			guint32 timestamp,
			GError **error)
{
...
#if GTK_CHECK_VERSION(3,22,0)
	GtkWindow *parent;
	parent = slategtk_scaffolding_window(slategtk_current_scaffolding());
...
/* exported interface documented in gtk/compat.h */
void slategtk_scrolled_window_add_with_viewport(GtkScrolledWindow *window,
		GtkWidget *child)
{
#if GTK_CHECK_VERSION(3,8,0)
...
/* exported interface documented in gtk/compat.h */
gboolean slategtk_icon_size_lookup_for_settings(GtkSettings *settings,
					     GtkIconSize size,
					     gint *width,
					     gint *height)
{
...
/* exported interface documented in gtk/compat.h */
guint
slategtk_builder_add_from_resource(GtkBuilder *builder,
