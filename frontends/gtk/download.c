...
enum {
	SLATEGTK_DOWNLOAD_PROGRESS,
	SLATEGTK_DOWNLOAD_INFO,
	SLATEGTK_DOWNLOAD_REMAINING,
	SLATEGTK_DOWNLOAD_SPEED,
	SLATEGTK_DOWNLOAD_PULSE,
	SLATEGTK_DOWNLOAD_STATUS,
	SLATEGTK_DOWNLOAD,

	SLATEGTK_DOWNLOAD_N_COLUMNS
};

typedef enum {
	SLATEGTK_DOWNLOAD_NONE,
	SLATEGTK_DOWNLOAD_WORKING,
	SLATEGTK_DOWNLOAD_ERROR,
	SLATEGTK_DOWNLOAD_COMPLETE,
	SLATEGTK_DOWNLOAD_CANCELED
} slategtk_download_status;

typedef enum {
	SLATEGTK_DOWNLOAD_PAUSE	= 1 << 0,
	SLATEGTK_DOWNLOAD_RESUME	= 1 << 1,
	SLATEGTK_DOWNLOAD_CANCEL	= 1 << 2,
	SLATEGTK_DOWNLOAD_CLEAR	= 1 << 3
} slategtk_download_actions;
...
struct gui_download_window {
	...
	slategtk_download_actions sensitivity;
	slategtk_download_status status;
...
typedef	void (*slategtk_download_selection_action)(struct gui_download_window *dl,
						void *user_data);

/**
 * context for a slategtk download window.
 */
...
static GtkTreeView* slategtk_download_tree_view_new(GtkBuilder *gladeFile)
{
...
static gint
slategtk_download_sort(GtkTreeModel *model,
		    GtkTreeIter *a,
		    GtkTreeIter *b,
		    gpointer userdata)
{
	...
	gtk_tree_model_get(model, a, SLATEGTK_DOWNLOAD, &dl1, -1);
	gtk_tree_model_get(model, b, SLATEGTK_DOWNLOAD, &dl2, -1);
...
static void
slategtk_download_sensitivity_update_buttons(slategtk_download_actions sensitivity)
{
	...
	gtk_widget_set_sensitive(GTK_WIDGET(dl_ctx.pause),
				 sensitivity & SLATEGTK_DOWNLOAD_PAUSE);
	gtk_widget_set_sensitive(GTK_WIDGET(dl_ctx.clear),
				 sensitivity & SLATEGTK_DOWNLOAD_CLEAR);
	gtk_widget_set_sensitive(GTK_WIDGET(dl_ctx.cancel),
				 sensitivity & SLATEGTK_DOWNLOAD_CANCEL);
	gtk_widget_set_sensitive(GTK_WIDGET(dl_ctx.resume),
				 sensitivity & SLATEGTK_DOWNLOAD_RESUME);
}


static void slategtk_download_sensitivity_evaluate(GtkTreeSelection *selection)
{
	GtkTreeIter iter;
	GList *rows;
	gboolean selected;
	GtkTreeModel *model;
	slategtk_download_actions sensitivity = 0;
	...
	if (selected) {
		...
		while (rows != NULL) {
			dl = rows->data;
			sensitivity |= (dl->sensitivity & SLATEGTK_DOWNLOAD_CLEAR);
			rows = rows->next;
		}
	}

	slategtk_download_sensitivity_update_buttons(sensitivity);
}
 ...
 */
static void
slategtk_download_gfunc__gtk_tree_path_free(gpointer data, gpointer user_data)
{
 ...
 */
static void
slategtk_download_gfunc__g_free(gpointer data, gpointer user_data)
{
	g_free(data);
}


static void slategtk_download_do(slategtk_download_selection_action action)
{
	GList *rows, *dls = NULL;
	GtkTreeModel *model;
	...
	if (gtk_tree_selection_count_selected_rows(dl_ctx.selection)) {
		...
		g_list_foreach(rows,
			       slategtk_download_gfunc__gtk_tree_path_free,
			       NULL);
		g_list_foreach(rows,
			       slategtk_download_gfunc__g_free,
			       NULL);
		g_list_free(rows);
	} else {
...
static gchar* slategtk_download_info_to_string(struct gui_download_window *dl)
{
	gchar *size_info;
	gchar *r;
	...
	if (dl->status != SLATEGTK_DOWNLOAD_ERROR) {
...
static gchar* slategtk_download_time_to_string(gint seconds)
{
	gint hours, minutes;
...
static void slategtk_download_store_update_item(struct gui_download_window *dl)
{
	gchar *info;
	char *speed;
	gchar *time;
	...
	info = slategtk_download_info_to_string(dl);
	...
	time = slategtk_download_time_to_string(dl->time_remaining);
	pulse = dl->status == SLATEGTK_DOWNLOAD_WORKING;
	...
	gtk_list_store_set(dl_ctx.store, &dl_ctx.iter,
			   SLATEGTK_DOWNLOAD_PULSE, pulse ? dl->progress : -1,
			   SLATEGTK_DOWNLOAD_PROGRESS, pulse ? 0 : dl->progress,
			   SLATEGTK_DOWNLOAD_INFO, info,
			   SLATEGTK_DOWNLOAD_SPEED, speed,
			   SLATEGTK_DOWNLOAD_REMAINING, time,
			   SLATEGTK_DOWNLOAD, dl,
			   -1);
...
static gboolean slategtk_download_update(gboolean force_update)
{
	/* Be sure we need to update */
	if (!slategtk_widget_get_visible(GTK_WIDGET(dl_ctx.window))) {
		return TRUE;
	}
	...
	for (list = dl_ctx.list; list != NULL; list = list->next) {
		...
		switch (dl->status) {
		case SLATEGTK_DOWNLOAD_WORKING:
			pulse_mode = TRUE;
			fallthrough;

		case SLATEGTK_DOWNLOAD_NONE:
			dl->speed = dl->size_downloaded /
				(elapsed - dl->start_time);
			if (dl->status == SLATEGTK_DOWNLOAD_NONE) {
		...
		case SLATEGTK_DOWNLOAD_COMPLETE:
		...
		if (update) {
			slategtk_download_store_update_item(dl);
		}
	}
...
static void
slategtk_download_store_clear_item(struct gui_download_window *dl,	void *user_data)
{
	if (dl->sensitivity & SLATEGTK_DOWNLOAD_CLEAR) {
		...
		slategtk_download_sensitivity_evaluate(dl_ctx.selection);
		slategtk_download_update(FALSE);
	}
}


static void
slategtk_download_tree_view_row_activated(GtkTreeView *tree,
				       GtkTreePath *path,
				       ...
				       gpointer data)
{
	...
	if (gtk_tree_model_get_iter(model, &iter, path)) {
		...
		slategtk_download_do(slategtk_download_store_clear_item);
	}
}


static void
slategtk_download_change_sensitivity(struct gui_download_window *dl,
				  slategtk_download_actions sensitivity)
{
...
static void
slategtk_download_change_status(struct gui_download_window *dl,
			     slategtk_download_status status)
{
	dl->status = status;
	if (status != SLATEGTK_DOWNLOAD_NONE) {
		gtk_tree_model_get_iter(GTK_TREE_MODEL(dl_ctx.store),
					&dl_ctx.iter,
					gtk_tree_row_reference_get_path(dl->row));
...
static void
slategtk_download_store_cancel_item(struct gui_download_window *dl,
				 void *user_data)
{
	if (dl->sensitivity & SLATEGTK_DOWNLOAD_CANCEL) {
		dl->speed = 0;
		dl->size_downloaded = 0;
		dl->progress = 0;
		dl->time_remaining = -1;
		slategtk_download_change_sensitivity(dl, SLATEGTK_DOWNLOAD_CLEAR);
		slategtk_download_change_status(dl, SLATEGTK_DOWNLOAD_CANCELED);

		download_context_abort(dl->ctx);

		g_unlink(download_context_get_filename(dl->ctx));

		slategtk_download_update(TRUE);
	}
}


static gboolean slategtk_download_hide(GtkWidget *window)
{
 ...
 */
static gchar*
slategtk_download_dialog_show(const gchar *filename,
			   const gchar *domain,
			   const gchar *size)
{
	...
	gtk_dialog_add_buttons(GTK_DIALOG(dialog),
			       SLATEGTK_STOCK_SAVE, GTK_RESPONSE_DOWNLOAD,
			       SLATEGTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
			       SLATEGTK_STOCK_SAVE_AS, GTK_RESPONSE_SAVE_AS,
			       NULL);
	...
	case GTK_RESPONSE_SAVE_AS: {
		...
		gtk_file_chooser_set_do_overwrite_confirmation
			(GTK_FILE_CHOOSER(dialog),
			 slateoption_bool(request_overwrite));
	...
	case GTK_RESPONSE_DOWNLOAD: {
		size_t destsize;
		destsize = strlen(slateoption_charp(downloads_directory)) +
		...
		snprintf(destination, destsize, "%s/%s",
			 slateoption_charp(downloads_directory), filename);
...
static gboolean slategtk_download_handle_error(GError *error)
{
	GtkWidget*dialog;
	gchar *message;
...
static void slategtk_download_store_create_item(struct gui_download_window *dl)
{
	slategtk_download_store_update_item(dl);
...
/**
 * Wrapper to GSourceFunc-ify slategtk_download_update.
 */
static gboolean
slategtk_download_gsourcefunc__slategtk_download_update(gpointer user_data)
{
	return slategtk_download_update(FALSE);
}
...
gui_download_window_create(download_context *ctx, struct gui_window *gui)
{
	...
	dl_ctx.parent =	slategtk_scaffolding_window(slategtk_get_scaffold(gui));
	...
	if (slateurl_has_component(url, SLATEURL_HOST)) {
		domain = g_strdup(lwc_string_data(slateurl_get_component(url, SLATEURL_HOST)));
	} else {
	...
	/* show the dialog */
	destination = slategtk_download_dialog_show(
	...
	download->status = SLATEGTK_DOWNLOAD_NONE;
	download->progress = 0;
	download->error = NULL;
	...
	if (slategtk_download_handle_error(download->error)) {
	...
	slategtk_download_change_sensitivity(download, SLATEGTK_DOWNLOAD_CANCEL);

	slategtk_download_store_create_item(download);
	slategtk_download_show(dl_ctx.parent);

	if (unknown_size) {
		slategtk_download_change_status(download, SLATEGTK_DOWNLOAD_WORKING);
	}

	if (dl_ctx.num_active == 0) {
		g_timeout_add(
			UPDATE_RATE,
			slategtk_download_gsourcefunc__slategtk_download_update,
			NULL);
	}
			 ...
			 unsigned int size)
{
	...
	if (status != G_IO_STATUS_NORMAL || dw->error != NULL) {
		...
		slategtk_download_change_sensitivity(dw, SLATEGTK_DOWNLOAD_CLEAR);
		slategtk_download_change_status(dw, SLATEGTK_DOWNLOAD_ERROR);
...
static void gui_download_window_done(struct gui_download_window *dw)
{
	...
	slategtk_download_change_sensitivity(dw, SLATEGTK_DOWNLOAD_CLEAR);
	slategtk_download_change_status(dw, SLATEGTK_DOWNLOAD_COMPLETE);

	if (slateoption_bool(downloads_clear)) {
		slategtk_download_store_clear_item(dw, NULL);
	} else {
		slategtk_download_update(TRUE);
	}
}
...
struct gui_download_table *slategtk_download_table = &download_table;
...
slateerror slategtk_download_init(void)
{
	...
	res = slategtk_builder_new_from_resname("downloads", &builder);
	...
	dl_ctx.tree = slategtk_download_tree_view_new(builder);

	dl_ctx.store = gtk_list_store_new(SLATEGTK_DOWNLOAD_N_COLUMNS,
					  G_TYPE_INT, /* % complete */
	...
	gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(dl_ctx.store),
					SLATEGTK_DOWNLOAD_STATUS,
					(GtkTreeIterCompareFunc)slategtk_download_sort, NULL, NULL);
	...
	g_signal_connect(G_OBJECT(dl_ctx.selection),
			 "changed",
			 G_CALLBACK(slategtk_download_sensitivity_evaluate),
			 NULL);

	g_signal_connect(dl_ctx.tree,
			 "row-activated",
			 G_CALLBACK(slategtk_download_tree_view_row_activated),
			 NULL);

	g_signal_connect_swapped(gtk_builder_get_object(builder, "buttonClear"),
				 "clicked",
				 G_CALLBACK(slategtk_download_do),
				 slategtk_download_store_clear_item);

	g_signal_connect_swapped(gtk_builder_get_object(builder, "buttonCancel"),
				 "clicked",
				 G_CALLBACK(slategtk_download_do),
				 slategtk_download_store_cancel_item);

	g_signal_connect(G_OBJECT(dl_ctx.window),
			 "delete-event",
			 G_CALLBACK(slategtk_download_hide),
			 NULL);
...
{
	slategtk_download_do(slategtk_download_store_cancel_item);
}
