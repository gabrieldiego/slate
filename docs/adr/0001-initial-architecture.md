...
Servo is vendored as a submodule at `third_party/servo` from `https://github.com/gabrieldiego/servo`. The initial `slate-rendering` crate exposes the `ServoBackend` boundary, records the vendored path, creates a Servo `WebView`, and captures a software-rendered Servo frame for the chrome. Full interactive compositor integration remains a follow-up implementation behind that boundary.
