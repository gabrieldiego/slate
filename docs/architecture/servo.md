...
Top-level HTTP(S), `ipfs://`, `ipns://`, and normalized `tor+http://` navigations first pass through Slate's in-process `broadwebd` service, then Servo renders HTML responses through the existing generated-document path. Fetched broadweb HTML receives a document `<base>` pointing at the effective broadweb document URL when the page did not define one, so relative page assets resolve back through broadweb routing. Kubo directory fallbacks use the effective `<path>/index.html` URL for this base. Servo's custom protocol registry now fetches `ipfs://`, `ipns://`, `tor+http://`, and `tor+https://` subresources through `broadwebd` as subresource requests, so CSS, JavaScript, images, and fonts do not create user download records. Non-2xx top-level responses become local response-error pages. Successful top-level non-HTML responses are spooled by broadwebd into profile-temporary download records and surfaced as download-ready pages with the saved path and size until the full downloads UI is connected. Remaining broadweb schemes such as `i2p`, `gemini`, and `magnet` still use a Servo custom protocol registry that returns a local placeholder page until real protocol adapters exist. Hostname-based private routes such as `.i2p` remain blocked before normal HTTP(S) routing because Servo does not allow overriding the `http` or `https` protocol handlers. `.onion` input is normalized to `tor+http://` or `tor+https://` before Servo sees it, and broadwebd still claims direct `http(s)://*.onion` defensively.
