...
The normal Settings surface exposes profile enrollment as a two-action
file-only flow: download the current profile's enrollment key file from an
enrolled browser, then select that file on another browser to import it.
First-use download may create and activate a local key when no sync metadata
...
Storage also exposes a reusable raw app-domain watcher for non-JSON settings
values. It initializes a persisted cursor at the current domain head, polls
bounded text batches, runs the app-owned apply callback, and records the cursor
only after that callback succeeds.
...
that framed provider client from a socketless `ServiceFrameEndpointTransport`,
so selected deferred endpoint refs can be exercised through the same transport
adapter shape future live broadweb transports must implement.
...
is published through both the socketless Kubo/IPNS model and a libp2p-shaped
provider. Composite discovery de-duplicates the same advertised peer across
protocol providers while preserving query priority order, so the preview can
exercise multiple discovery models without selecting the same retention
provider twice. The signed broadweb-style advertisement can therefore be
trusted and selected while its discovered multiaddr remains pending protocol
materialization; the socketless fixture handle used for the local run is
reported as runnable stored-provider state, not as the advertised network
endpoint. Preview report DTOs and string summary adapters live in
...
low-memory profile. Preview JSON also exposes the modeled `discovery_protocols`
list so the Settings page can report which local-only broadweb discovery routes
were exercised.
