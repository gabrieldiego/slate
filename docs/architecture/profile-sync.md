...
The tombstone-tail fixture uses the same cursor sequence for Chat and Downloads
deletions, so apps can observe decoded deletion payloads and only then
acknowledge the tail revision.
...
Chat, Contacts, Downloads, Files, and Storage, and Chat/Downloads tombstone
tails to verify incremental typed changes use the same trusted device-head
path. These tests use `InProcessBroadwebNetwork`, so app-domain create, update,
and delete propagation is verified without
