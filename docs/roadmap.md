...
- The `slate-profile-sync` runtime bridge now also has full-snapshot
  ...
  Storage, and verify Chat/Downloads/Files post-snapshot tombstone tails delete
  stale typed rows on the receiver.
...
- The `slate-profile-sync` runtime bridge now verifies received typed Calendar,
  ...
  batch. Chat, Downloads, and Files tombstone tails are covered through the
  same typed watcher path, proving deletions are observable before the app
  advances its cursor.
