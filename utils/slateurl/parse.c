 ...
 */
static bool slateurl__is_unreserved(unsigned char c)
{
 ...
 */
static bool slateurl__is_no_escape(unsigned char c)
{
 ...
 */
static void slateurl__get_string_markers(const char * const url_s,
		struct url_markers *markers, bool joining)
{
		...
		if (*pos == ':') {
			...
			if (off == SLEN("http") &&
					(((*(pos - off + 0) == 'h') ||
					 ...
					 ((*(pos - off + 3) == 'p') ||
					  (*(pos - off + 3) == 'P')))) {
				marker.scheme_type = SLATEURL_SCHEME_HTTP;
				is_http = true;
					...
					(((*(pos - off + 0) == 'h') ||
					 ...
					 ((*(pos - off + 4) == 's') ||
					  (*(pos - off + 4) == 'S')))) {
				marker.scheme_type = SLATEURL_SCHEME_HTTPS;
				is_http = true;
					...
					(((*(pos - off + 0) == 'f') ||
					 ...
					 ((*(pos - off + 3) == 'e') ||
					  (*(pos - off + 3) == 'E')))) {
				marker.scheme_type = SLATEURL_SCHEME_FILE;
					...
					(((*(pos - off + 0) == 'f') ||
					 ...
					 ((*(pos - off + 2) == 'p') ||
					  (*(pos - off + 2) == 'P')))) {
				marker.scheme_type = SLATEURL_SCHEME_FTP;
					...
					(((*(pos - off + 0) == 'm') ||
					 ...
					 ((*(pos - off + 5) == 'o') ||
					  (*(pos - off + 5) == 'O')))) {
				marker.scheme_type = SLATEURL_SCHEME_MAILTO;
					...
					(((*(pos - off + 0) == 'd') ||
					 ...
					 ((*(pos - off + 3) == 'a') ||
					  (*(pos - off + 3) == 'A')))) {
				marker.scheme_type = SLATEURL_SCHEME_DATA;
			}
	...
	if ((*pos == '/' && *(pos + 1) == '/') ||
		...
		while (*pos != '\0') {
			...
			} else if (marker.scheme_type != SLATEURL_SCHEME_MAILTO &&
 ...
 */
static size_t slateurl__remove_dot_segments(char *path, char *output)
{
	char *path_pos = path;
 ...
 */
static size_t slateurl__get_longest_section(struct url_markers *m)
{
 ...
 */
static slateerror slateurl__create_from_section(const char * const url_s,
		const enum url_sections section,
		const struct url_markers *pegs,
		char *pos_norm,
		struct slateurl_components *url)
{
	slateerror ret;
	int ascii_offset;
	int start = 0;
	...
	for (; pos < url_s + end; pos++) {
		if (*pos == '%' && (pos + 2 < url_s + end)) {
			...
			if ((section != URL_SCHEME && section != URL_HOST) &&
				(slateurl__is_unreserved(ascii_offset) == false)) {
			...
			*(pos_norm++) = (char)ascii_offset;
			...
			length -= 2;
				...
				(slateurl__is_no_escape(*pos) == false)) {
	...
	case URL_CREDENTIALS:
		...
		if (url->scheme_type == SLATEURL_SCHEME_FILE) {
			break;
		}
	...
	case URL_HOST:
		...
		if (length != 0) {
			...
			if (pegs->at < pegs->colon_first &&
				 ...
				 * separator */
				flags |= SLATEURL_F_NO_PORT;
			}

			if (!(flags & SLATEURL_F_NO_PORT)) {
	...
	case URL_PATH:
		if (length != 0) {
			if (lwc_intern_string(norm_start, length,
				...
				url->scheme_type != SLATEURL_SCHEME_MAILTO) ||
				url->scheme_type == SLATEURL_SCHEME_FILE) {
...
/**
 * Get slateurl string info; total length, component lengths, & components present
 *
 ...
 */
static void slateurl__get_string_data(const struct slateurl_components *url,
		slateurl_component parts, size_t *url_l,
		struct slateurl_component_lengths *lengths,
		enum slateurl_string_flags *pflags)
{
	enum slateurl_string_flags flags = *pflags;
	*url_l = 0;

	/* Intersection of required parts and available parts gives
	 * the output parts */
	if (url->scheme && parts & SLATEURL_SCHEME) {
		flags |= SLATEURL_F_SCHEME;

		lengths->scheme = lwc_string_length(url->scheme);
		*url_l += lengths->scheme;
	}

	if (url->username && parts & SLATEURL_USERNAME) {
		flags |= SLATEURL_F_USERNAME;

		lengths->username = lwc_string_length(url->username);
		*url_l += lengths->username;
	}

	if (url->password && parts & SLATEURL_PASSWORD) {
		flags |= SLATEURL_F_PASSWORD;

		lengths->password = lwc_string_length(url->password);
		*url_l += SLEN(":") + lengths->password;
	}

	if (url->host && parts & SLATEURL_HOST) {
		flags |= SLATEURL_F_HOST;

		lengths->host = lwc_string_length(url->host);
		*url_l += lengths->host;
	}

	if (url->port && parts & SLATEURL_PORT) {
		flags |= SLATEURL_F_PORT;

		lengths->port = lwc_string_length(url->port);
		*url_l += SLEN(":") + lengths->port;
	}

	if (url->path && parts & SLATEURL_PATH) {
		flags |= SLATEURL_F_PATH;

		lengths->path = lwc_string_length(url->path);
		*url_l += lengths->path;
	}

	if (url->query && parts & SLATEURL_QUERY) {
		flags |= SLATEURL_F_QUERY;

		lengths->query = lwc_string_length(url->query);
		*url_l += lengths->query;
	}

	if (url->fragment && parts & SLATEURL_FRAGMENT) {
		flags |= SLATEURL_F_FRAGMENT;
	...
	if ((flags & SLATEURL_F_SCHEME) && (parts > SLATEURL_SCHEME)) {
		flags |= SLATEURL_F_SCHEME_PUNCTUATION;

		*url_l += SLEN(":");
	}

	if ((flags & SLATEURL_F_SCHEME) && (flags > SLATEURL_F_SCHEME) &&
			url->path && lwc_string_data(url->path)[0] == '/') {
		flags |= SLATEURL_F_AUTHORITY_PUNCTUATION;

		*url_l += SLEN("//");
	}

	if ((flags & (SLATEURL_F_USERNAME | SLATEURL_F_PASSWORD)) &&
				flags & SLATEURL_F_HOST) {
		flags |= SLATEURL_F_CREDENTIALS_PUNCTUATION;

		*url_l += SLEN("@");
	}

	/* spanned query question mark */
	if ((flags & ~(SLATEURL_F_QUERY | SLATEURL_F_FRAGMENT)) &&
	    (flags & SLATEURL_F_QUERY)) {
		flags |= SLATEURL_F_QUERY_PUNCTUATION;

		*url_l += SLEN("?");
	}

	/* spanned fragment hash mark */
	if ((flags & ~SLATEURL_F_FRAGMENT) && (flags & SLATEURL_F_FRAGMENT)) {
		flags |= SLATEURL_F_FRAGMENT_PUNCTUATION;
 ...
 */
static void slateurl__get_string(const struct slateurl_components *url, char *url_s,
		struct slateurl_component_lengths *l,
		enum slateurl_string_flags flags)
{
	char *pos;
	...
	if (flags & SLATEURL_F_SCHEME) {
		memcpy(pos, lwc_string_data(url->scheme), l->scheme);
		pos += l->scheme;
	}

	if (flags & SLATEURL_F_SCHEME_PUNCTUATION) {
		*(pos++) = ':';
	}

	if (flags & SLATEURL_F_AUTHORITY_PUNCTUATION) {
		*(pos++) = '/';
		*(pos++) = '/';
	}

	if (flags & SLATEURL_F_USERNAME) {
		memcpy(pos, lwc_string_data(url->username), l->username);
		pos += l->username;
	}

	if (flags & SLATEURL_F_PASSWORD) {
		*(pos++) = ':';
		memcpy(pos, lwc_string_data(url->password), l->password);
		pos += l->password;
	}

	if (flags & SLATEURL_F_CREDENTIALS_PUNCTUATION) {
		*(pos++) = '@';
	}

	if (flags & SLATEURL_F_HOST) {
		memcpy(pos, lwc_string_data(url->host), l->host);
		pos += l->host;
	}

	if (flags & SLATEURL_F_PORT) {
		*(pos++) = ':';
		memcpy(pos, lwc_string_data(url->port), l->port);
		pos += l->port;
	}

	if (flags & SLATEURL_F_PATH) {
		memcpy(pos, lwc_string_data(url->path), l->path);
		pos += l->path;
	}

	if (flags & SLATEURL_F_QUERY) {
		if (flags & SLATEURL_F_QUERY_PUNCTUATION)
			*(pos++) = '?';
		memcpy(pos, lwc_string_data(url->query), l->query);
		pos += l->query;
	}

	if (flags & SLATEURL_F_FRAGMENT) {
		if (flags & SLATEURL_F_FRAGMENT_PUNCTUATION)
			*(pos++) = '#';
...
{
	struct slateurl_component_lengths str_len = { 0, 0, 0, 0,  0, 0, 0, 0 };
	enum slateurl_string_flags str_flags = 0;
	size_t url_l;
	char *url_s;
	...
	slateurl__get_string_data(components, parts, &url_l,
			&str_len, &str_flags);
	...
	/* Copy the required parts into the url string */
	slateurl__get_string(components, url_s + pre_padding, &str_len, str_flags);
 ...
 * \return SLATEERROR_OK if the hostname is valid
 */
static slateerror slateurl__check_host_valid(lwc_string *host)
{
...
slateerror slateurl_create(const char * const url_s, slateurl **url)
{
	...
	struct slateurl_components c;
	size_t length;
	char *buff;
	slateerror e = SLATEERROR_OK;
	bool match;

	assert(url_s != NULL);

	/* Peg out the URL sections */
	slateurl__get_string_markers(url_s, &m, false);

	/* Get the length of the longest section */
	length = slateurl__get_longest_section(&m);
	...
	e |= slateurl__create_from_section(url_s, URL_SCHEME, &m, buff, &c);
	e |= slateurl__create_from_section(url_s, URL_CREDENTIALS, &m, buff, &c);
	e |= slateurl__create_from_section(url_s, URL_HOST, &m, buff, &c);
	e |= slateurl__create_from_section(url_s, URL_PATH, &m, buff, &c);
	e |= slateurl__create_from_section(url_s, URL_QUERY, &m, buff, &c);
	e |= slateurl__create_from_section(url_s, URL_FRAGMENT, &m, buff, &c);

	/* Finished with buffer */
	free(buff);

	if (e != SLATEERROR_OK) {
		slateurl__components_destroy(&c);
		return SLATEERROR_NOMEM;
	}
	...
	if ((lwc_string_isequal(c.scheme, corestring_lwc_http,
		...
		if (slateurl__check_host_valid(c.host) != SLATEERROR_OK) {
			slateurl__components_destroy(&c);
			return SLATEERROR_BAD_URL;
		}
	}

	e = slateurl__components_to_string(&c, SLATEURL_WITH_FRAGMENT,
			offsetof(slateurl, string), (char **)url, &length);
	if (e != SLATEERROR_OK) {
		return e;
	}
...
slateerror slateurl_join(const slateurl *base, const char *rel, slateurl **joined)
{
	...
	enum {
		SLATEURL_F_REL		=  0,
		SLATEURL_F_BASE_SCHEME	= (1 << 0),
		SLATEURL_F_BASE_AUTHORITY	= (1 << 1),
		SLATEURL_F_BASE_PATH	= (1 << 2),
		SLATEURL_F_MERGED_PATH	= (1 << 3),
		SLATEURL_F_BASE_QUERY	= (1 << 4)
	} joined_parts;
	...
	NSLOG(netsurf, DEEPDEBUG, "base: \"%s\", rel: \"%s\"",
			slateurl_access(base), rel);

	/* Peg out the URL sections */
	slateurl__get_string_markers(rel, &m, true);
	 ...
	 */
	if (joined_parts & SLATEURL_F_MERGED_PATH) {
		/* Need to merge paths */
	...
	if (joined_parts & SLATEURL_F_BASE_SCHEME) {
		c.scheme_type = base->components.scheme_type;

		c.scheme = slateurl__component_copy(base->components.scheme);
	} else {
		c.scheme_type = m.scheme_type;

		error = slateurl__create_from_section(rel, URL_SCHEME, &m,	buff, &c);
	...
	if (joined_parts & SLATEURL_F_BASE_AUTHORITY) {
		c.username = slateurl__component_copy(base->components.username);
		c.password = slateurl__component_copy(base->components.password);
		c.host = slateurl__component_copy(base->components.host);
		c.port = slateurl__component_copy(base->components.port);
	} else {
		error = slateurl__create_from_section(rel, URL_CREDENTIALS, &m,
						   buff, &c);
		if (error == SLATEERROR_OK) {
			error = slateurl__create_from_section(rel, URL_HOST, &m,
							   buff, &c);
		}
		if (error != SLATEERROR_OK) {
			free(buff);
	...
	if (joined_parts & SLATEURL_F_BASE_PATH) {
		c.path = slateurl__component_copy(base->components.path);

	} else if (joined_parts & SLATEURL_F_MERGED_PATH) {
		...
		*buff_pos++ = '\0';
		...
		new_length = slateurl__remove_dot_segments(buff, buff_pos);

		m_path.path = 0;
		m_path.query = new_length;

		buff_start = buff_pos + new_length;
		error = slateurl__create_from_section(buff_pos, URL_PATH, &m_path,
				buff_start, &c);
		if (error != SLATEERROR_OK) {
			free(buff);
			return error;
	...
	if (joined_parts & SLATEURL_F_BASE_QUERY) {
		c.query = slateurl__component_copy(base->components.query);
	} else {
		error = slateurl__create_from_section(rel, URL_QUERY, &m,
				buff, &c);
		if (error != SLATEERROR_OK) {
			free(buff);
			return error;
	...
	error = slateurl__create_from_section(rel, URL_FRAGMENT, &m, buff, &c);
	...
	error = slateurl__components_to_string(&c, SLATEURL_WITH_FRAGMENT,
			offsetof(slateurl, string), (char **)joined, &length);
	if (error != SLATEERROR_OK) {
		return error;
	}
