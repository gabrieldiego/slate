...
slateerror slate_init(const char *store_path)
{
	slateerror ret;
	...
	ret = slatecolor_update();
	if (ret != SLATEERROR_OK)
		return ret;

	/* set up cache limits based on the memory cache size option */
	hlcache_parameters.llcache.limit = slateoption_int(memory_cache_size);
	...
	hlcache_parameters.llcache.fetch_attempts = slateoption_uint(max_retried_fetches);
	...
	hlcache_parameters.llcache.store.limit = slateoption_uint(disc_cache_size);
	...
	hlcache_parameters.llcache.store.path =
		slateoption_charp(disc_cache_path) ?
		slateoption_charp(disc_cache_path) :
		store_path;
