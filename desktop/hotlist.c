 ...
 */
static slateerror hotlist_get_temp_path(const char *path, char **temp_path)
{
 ...
 * \return SLATEERROR_OK on success, or appropriate error otherwise
 */
static slateerror hotlist_save(const char *path)
{
 ...
 * \return SLATEERROR_OK on success, or appropriate error otherwise
 */
static slateerror hotlist_schedule_save(void)
{
	if (hl_ctx.save_scheduled == false && hl_ctx.save_path != NULL) {
		slateerror err = guit->misc->schedule(10 * 1000,
 ...
 */
static slateerror hotlist_create_treeview_field_visits_data(
 ...
 * \return SLATEERROR_OK on success, appropriate error otherwise
 */
static slateerror hotlist_create_treeview_field_data(
		...
		const struct url_data *data)
{
	...
	e->data[HL_URL].value = slateurl_access(e->url);
	e->data[HL_URL].value_len = slateurl_length(e->url);

	/* "Title" field */
	if (title == NULL) {
		/* Title not provided; use one from URL data */
		title = (data->title != NULL) ?
				data->title : slateurl_access(e->url);
	}
 ...
 * hotlist table
 */
static slateerror hotlist_entry_insert(struct hotlist_entry *e,
 ...
 * \return SLATEERROR_OK on success, or appropriate error otherwise
 */
static slateerror hotlist_create_entry(slateurl *url, const char *title,
 ...
 * \return SLATEERROR_OK on success, or appropriate error otherwise
 */
static slateerror hotlist_add_entry_internal(slateurl *url, const char *title,
 ...
 * \return SLATEERROR_OK on success, or appropriate error otherwise
 */
static slateerror hotlist_add_folder_internal(
...
static slateerror hotlist_tree_node_folder_cb(
...
hotlist_tree_node_entry_cb(struct treeview_node_msg msg, void *data)
{
	...
	case TREE_MSG_NODE_EDIT:
		if (lwc_string_isequal(hl_ctx.fields[HL_TITLE].field,
			...
			err = slateurl_create(msg.data.node_edit.text, &url);
			if (err == SLATEERROR_OK) {
				old_url = e->url;

				e->url = url;
				e->data[HL_URL].value = slateurl_access(url);
				e->data[HL_URL].value_len = slateurl_length(e->url);

				treeview_update_node_entry(hl_ctx.tree,
						   e->entry, e->data, e);
				slateurl_unref(old_url);
 ...
 * \return SLATEERROR_OK on success, or appropriate error otherwise
 */
static slateerror hotlist_load_entry(dom_node *li, hotlist_load_ctx *ctx)
{
	dom_node *a;
	...
	/* Need to get URL as a slateurl object */
	err = slateurl_create(dom_string_data(url1), &url);
 ...
 * \param ctx		Our hotlist loading context.
 */
static slateerror hotlist_load_directory_cb(dom_node *node, void *ctx);
 ...
 * \return SLATEERROR_OK on success, or appropriate error otherwise
 */
static slateerror hotlist_load_directory(dom_node *ul, hotlist_load_ctx *ctx)
{
	assert(ul != NULL);
	assert(ctx != NULL);
...
slateerror hotlist_load_directory_cb(dom_node *node, void *ctx)
{
 ...
 * \return SLATEERROR_OK on success, or appropriate error otherwise
 */
static slateerror hotlist_load(const char *path, bool *loaded)
{
 ...
 * \return SLATEERROR_OK on success, or appropriate error otherwise
 */
static slateerror hotlist_generate(void)
{
	int i;
	struct hotlist_folder *f;
	treeview_node *e;
	...
	static const struct {
		...
		{ "https://www.slate-browser.org/",
				"HotlistHomepage" },
		{ "https://www.slate-browser.org/downloads/",
				"HotlistDownloads" },
		{ "https://www.slate-browser.org/documentation",
				"HotlistDocumentation" },
		{ "https://www.slate-browser.org/contact",
				"HotlistContact" }
	};
	...
	for (i = n_entries - 1; i >= 0; i--) {
		/* Get URL as slateurl object */
		err = slateurl_create(default_entries[i].url, &url);
		if (err != SLATEERROR_OK) {
			return SLATEERROR_NOMEM;
		}
...
static slateerror hotlist_export_enter_cb(void *ctx, void *node_data,
...
static slateerror hotlist_export_leave_cb(void *ctx, void *node_data,
		enum treeview_node_type type, bool *abort)
{
	struct treeview_export_walk_ctx *tw = ctx;
...
static slateerror hotlist_iterate_enter_cb(void *ctx, void *node_data,
		enum treeview_node_type type, bool *abort)
{
...
static slateerror hotlist_iterate_leave_cb(void *ctx, void *node_data,
		enum treeview_node_type type, bool *abort)
{
	struct hotlist_iterate_ctx *data = ctx;
 ...
 * \return SLATEERROR_OK on success, or appropriate error otherwise
 */
static slateerror hotlist_initialise_entry_fields(void)
{
	int i;
	const char *label;
 ...
 * \return SLATEERROR_OK on success, or appropriate error otherwise
 */
static slateerror hotlist_populate(const char *path)
{
	slateerror err;
	bool loaded;
...
/* Exported interface, documented in hotlist.h */
slateerror hotlist_add_url(slateurl *url)
{
	treeview_node *entry;
	slateerror err;
...
static slateerror hotlist_has_url_walk_cb(void *ctx, void *node_data,
		enum treeview_node_type type, bool *abort)
{
	...
	if (type == TREE_NODE_ENTRY) {
		...
		if (slateurl_compare(e->url, tw->url, SLATEURL_COMPLETE) == true) {
...
/* Exported interface, documented in hotlist.h */
bool hotlist_has_url(slateurl *url)
{
	slateerror err;
...
/** Callback for treeview_walk */
static slateerror hotlist_remove_url_walk_cb(void *ctx, void *node_data,
		enum treeview_node_type type, bool *abort)
{
...
/* Exported interface, documented in hotlist.h */
void hotlist_remove_url(slateurl *url)
{
	slateerror err;
...
/** Callback for treeview_walk */
static slateerror hotlist_update_url_walk_cb(void *ctx, void *node_data,
		enum treeview_node_type type, bool *abort)
{
...
/* Exported interface, documented in hotlist.h */
void hotlist_update_url(slateurl *url)
{
	slateerror err;
...
/* Exported interface, documented in hotlist.h */
slateerror hotlist_add_entry(slateurl *url, const char *title, bool at_y, int y)
{
	slateerror err;
	treeview_node *entry;
	...
	if (url == NULL) {
		err = slateurl_create("https://netsurf-browser.org/", &url);
		if (err != SLATEERROR_OK) {
			return err;
		}
		assert(url != NULL);
...
/* Exported interface, documented in hotlist.h */
bool hotlist_get_selection(slateurl **url, const char **title)
{
	struct hotlist_entry *e;
