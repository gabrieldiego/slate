                      ...
                      struct ir_operation_argument_entry *membere)
{
        ...
        case WEBIDL_TYPE_STRING:
                outputf(outc,
                        "const char *\n"
                        "%s_%s_get_%s(slatejs_context *ctx, slatejs_idx_t idx)\n"
                        "{\n",
                ...
                outputf(outc,
                        "\t/* ... obj@idx ... */\n"
                        "\tslatejs_get_prop_string(ctx, idx, \"%s\");\n"
                        "\t/* ... obj@idx ... value/undefined */\n"
                        "\tif (!slatejs_is_undefined(ctx, -1)) {\n"
                        "\t\t/* Note, this throws a slatejs_error if it's not a string */\n"
                        "\t\tret = slatejs_require_string(ctx, -1);\n"
                        "\t}\n"
                        "\tslatejs_pop(ctx);\n"
                        "\treturn ret;\n"
                        "}\n\n",
        ...
        case WEBIDL_TYPE_BOOL:
                ...
                if (defl == NULL) {
                        outputf(outc,
                                "\tslatejs_bool_t ret = false; /* No default */\n");
                } else {
                outputf(outc,
                        "\tslatejs_bool_t ret = %s; /* Default value of %s */\n",
                        defl, membere->name);
                }
                ...
                outputf(outc,
                        ...
                        "\t/* ... obj@idx ... value/undefined */\n"
                        "\tif (!slatejs_is_undefined(ctx, -1)) {\n"
                        "\t\t/* Note, this throws a slatejs_error if it's not a boolean */\n"
                        "\t\tret = slatejs_require_boolean(ctx, -1);\n"
                        "\t}\n"
                        "\tslatejs_pop(ctx);\n"
                        "\treturn ret;\n"
                        "}\n\n",
        ...
        case WEBIDL_TYPE_LONGLONG:
                ...
                if (defl == NULL) {
                        outputf(outc,
                                "\tslatejs_int_t ret = 0; /* No default */\n");
                } else {
                        outputf(outc,
                                "\tslatejs_int_t ret = %s; /* Default value of %s */\n",
                                defl, membere->name);
                }
                ...
                outputf(outc,
                        ...
                        "\t/* ... obj@idx ... value/undefined */\n"
                        "\tif (!slatejs_is_undefined(ctx, -1)) {\n"
                        "\t\t/* Note, this throws a slatejs_error if it's not a int */\n"
                        "\t\tret = slatejs_require_int(ctx, -1);\n"
                        "\t}\n"
                        "\tslatejs_pop(ctx);\n"
                        "\treturn ret;\n"
                        "}\n\n",
        ...
        case WEBIDL_TYPE_DOUBLE:
                ...
                outputf(outc,
                        "{\n"
                        "\tslatejs_double_t ret = %s; /* Default value of %s */\n"
                        "\t/* ... obj@idx ... */\n"
                        "\tslatejs_get_prop_string(ctx, idx, \"%s\");\n",
                ...
                outputf(outc,
                        "\t/* ... obj@idx ... value/undefined */\n"
                        "\tif (!slatejs_is_undefined(ctx, -1)) {\n"
                        "\t\t/* Note, this throws a slatejs_error if it's not a number */\n"
                        "\t\tret = slatejs_require_number(ctx, -1);\n"
                        "\t}\n"
                        "\tslatejs_pop(ctx);\n"
                        "\treturn ret;\n"
                        "}\n\n");
                          ...
                          struct ir_operation_argument_entry *membere)
{
        ...
        case WEBIDL_TYPE_STRING:
                outputf(outc,
                        "const char *%s_%s_get_%s(slatejs_context *ctx, slatejs_idx_t idx);\n",
                        DLPFX, dictionarye->class_name, membere->name);
                break;

        case WEBIDL_TYPE_BOOL:
                outputf(outc,
                        "slatejs_bool_t %s_%s_get_%s(slatejs_context *ctx, slatejs_idx_t idx);\n",
                        DLPFX, dictionarye->class_name, membere->name);
                break;
        ...
        case WEBIDL_TYPE_LONGLONG:
                outputf(outc,
                        "slatejs_int_t %s_%s_get_%s(slatejs_context *ctx, slatejs_idx_t idx);\n",
                        DLPFX, dictionarye->class_name, membere->name);
                break;
        ...
        case WEBIDL_TYPE_DOUBLE:
                outputf(outc,
                        "slatejs_double_t %s_%s_get_%s(slatejs_context *ctx, slatejs_idx_t idx);\n",
                        DLPFX, dictionarye->class_name, membere->name);
                break;
