...
#define DLPFX "slatejs_bind"

#define MAGICPFX "\\xFF\\xFFSLATE_QUICKJS_"


/**
 * generate a quickjs prototype name
 */
...
output_create_private(struct opctx *outc, char *class_name)
{
        ...
        outputf(outc,
                "\tslatejs_push_pointer(ctx, priv);\n");
        outputf(outc,
                "\tslatejs_put_prop_string(ctx, 0, %s_magic_string_private);\n\n",
                DLPFX);
...
output_safe_get_private(struct opctx *outc, char *class_name, int idx)
{
        outputf(outc,
                "\t%s_private_t *priv;\n", class_name);
        outputf(outc,
                "\tslatejs_get_prop_string(ctx, %d, %s_magic_string_private);\n",
                idx, DLPFX);
        outputf(outc,
...
static int output_get_prototype(struct opctx *outc, const char *interface_name)
{
        ...
        outputf(outc,
                "\t/* get prototype */\n");
        outputf(outc,
                "\tslatejs_get_global_string(ctx, %s_magic_string_prototypes);\n",
                DLPFX);
        outputf(outc,
                "\tslatejs_get_prop_string(ctx, -1, \"%s\");\n",
                proto_name);
        outputf(outc,
...
static int output_set_destructor(struct opctx *outc, char *class_name, int idx)
{
        ...
        outputf(outc,
                "\tslatejs_dup(ctx, %d);\n", idx);
        outputf(outc,
                "\tslatejs_push_c_function(ctx, %s_%s___destructor, 1);\n",
                DLPFX, class_name);
        outputf(outc,
                "\tslatejs_set_finalizer(ctx, -2);\n");
        outputf(outc,
                "\tslatejs_pop(ctx);\n\n");
...
output_set_constructor(struct opctx *outc, char *class_name, int idx, int argc)
{
        ...
        outputf(outc,
                "\tslatejs_dup(ctx, %d);\n", idx);
        outputf(outc,
                "\tslatejs_push_c_function(ctx, %s_%s___constructor, %d);\n",
                DLPFX, class_name, 1 + argc);
        outputf(outc,
                "\tslatejs_put_prop_string(ctx, -2, \"%sINIT\");\n",
                MAGICPFX);
        outputf(outc,
                "\tslatejs_pop(ctx);\n\n");
...
output_dump_stack(struct opctx *outc)
{
        if (options->dbglog) {
                ...
                outputf(outc,
                        "\tNSLOG(quickjs, DEEPDEBUG, \"Stack: %%s\", slatejs_to_string(ctx, -1));\n");
                outputf(outc,
                        "\tslatejs_pop(ctx);\n");
        }
        return 0;
                  ...
                  const char *method)
{
        ...
        outputf(outc,
                "\tslatejs_dup(ctx, 0);\n");
        outputf(outc,
                "\tslatejs_push_string(ctx, \"%s\");\n", method);
        outputf(outc,
                "\tslatejs_push_c_function(ctx, %s_%s_%s, SLATEJS_VARARGS);\n",
                            ...
                            const char *property)
{
        ...
        outputf(outc,
                "\tslatejs_push_string(ctx, \"%s\");\n", property);
        outputf(outc,
                "\tslatejs_push_c_function(ctx, %s_%s_%s_getter, 0);\n",
                DLPFX, class_name, property);
        outputf(outc,
                "\tslatejs_push_c_function(ctx, %s_%s_%s_setter, 1);\n",
                DLPFX, class_name, property);
        output_dump_stack(outc);
        outputf(outc,
                "\tslatejs_def_prop(ctx, -4, SLATEJS_DEFPROP_HAVE_GETTER |\n");
        outputf(outc,
                "\t\tSLATEJS_DEFPROP_HAVE_SETTER |\n");
        outputf(outc,
                "\t\tSLATEJS_DEFPROP_HAVE_ENUMERABLE | SLATEJS_DEFPROP_ENUMERABLE |\n");
        outputf(outc,
                "\t\tSLATEJS_DEFPROP_HAVE_CONFIGURABLE);\n");
        outputf(outc,
                "\tslatejs_pop(ctx);\n\n");
                            ...
                            const char *property)
{
        ...
        output_dump_stack(outc);
        outputf(outc,
                "\tslatejs_def_prop(ctx, -3, SLATEJS_DEFPROP_HAVE_GETTER |\n");
        outputf(outc,
                "\t\tSLATEJS_DEFPROP_HAVE_ENUMERABLE | SLATEJS_DEFPROP_ENUMERABLE |\n");
        outputf(outc,
                "\t\tSLATEJS_DEFPROP_HAVE_CONFIGURABLE);\n");
                              ...
                              int value)
{
        ...
        outputf(outc,
                "\tslatejs_push_string(ctx, \"%s\");\n", constant_name);
        outputf(outc,
                "\tslatejs_push_int(ctx, %d);\n", value);
        outputf(outc,
                "\tslatejs_def_prop(ctx, -3, SLATEJS_DEFPROP_HAVE_VALUE |\n");
        outputf(outc,
                "\t\t     SLATEJS_DEFPROP_HAVE_WRITABLE |\n");
        outputf(outc,
                "\t\t     SLATEJS_DEFPROP_HAVE_ENUMERABLE |\n");
...
output_get_method_private(struct opctx *outc, char *class_name, bool is_global)
{
        ...
        outputf(outc,
                "\tslatejs_get_prop_string(ctx, -1, %s_magic_string_private);\n",
                DLPFX);
        outputf(outc,
                "\tpriv = slatejs_get_pointer(ctx, -1);\n");
        outputf(outc,
        ...
        if (options->dbglog) {
                outputf(outc,
                        "\t\tNSLOG(quickjs, INFO, \"priv failed\");\n");
        }
        outputf(outc,
...
output_interface_constructor(struct opctx *outc, struct ir_entry *interfacee)
{
        ...
        outputf(outc,
                "static slatejs_ret_t %s_%s___constructor(slatejs_context *ctx)\n",
		...
		case IR_INIT_ARG_BOOL:
			outputf(outc,
				", slatejs_get_boolean(ctx, %d)",
				init_argc);
			break;
		case IR_INIT_ARG_UNSIGNED:
		case IR_INIT_ARG_INT:
			outputf(outc,
				", slatejs_get_int(ctx, %d)",
				init_argc);
			break;
		case IR_INIT_ARG_POINTER:
			outputf(outc,
				", slatejs_get_pointer(ctx, %d)",
				init_argc);
			break;
		}
        }
...
output_interface_destructor(struct opctx *outc, struct ir_entry *interfacee)
{
        ...
        outputf(outc,
                "static slatejs_ret_t %s_%s___destructor(slatejs_context *ctx)\n",
                DLPFX, interfacee->class_name);
        outputf(outc,
                "{\n");
                                  ...
                                  struct genbind_node *init_node)
{
        ...
        outputf(outc,
                "void %s_%s___init(slatejs_context *ctx, %s_private_t *priv",
                      ...
                      struct ir_entry *inherite)
{
        ...
        if (options->dbglog) {
                outputf(outc,
                        "\tNSLOG(quickjs, INFO, \"Initialise %%p (priv=%%p)\", slatejs_get_heapptr(ctx, 0), priv);\n" );
        }
                      ...
                      struct ir_entry *inherite)
{
        ...
        outputf(outc,
                "void %s_%s___fini(slatejs_context *ctx, %s_private_t *priv)\n",
                DLPFX, interfacee->class_name, interfacee->class_name);
        outputf(outc, "{\n");
        ...
        if (options->dbglog) {
                outputf(outc,
                        "\tNSLOG(quickjs, INFO, \"Finalise %%p\", slatejs_get_heapptr(ctx, 0));\n" );
        }
                               ...
                               struct ir_entry *interfacee)
{
        ...
        for (idx = 0; idx < ir->entryc; idx++) {
                ...
                if (entry->type == IR_ENTRY_TYPE_INTERFACE) {
                        ...
                        outputf(outc,
                                "\tslatejs_bind_inject_not_ctr(ctx, 0, \"%s\");\n",
                                entry->name);
                }
        }
        return 0;
                           ...
                           struct ir_entry *inherite)
{
        ...
        outputf(outc,
                "slatejs_ret_t %s_%s___proto(slatejs_context *ctx, void *udata)\n",
                DLPFX, interfacee->class_name);
        outputf(outc, "{\n");
                                   ...
                                   struct ir_operation_entry *operatione)
{
        ...
        outputf(outc,
                "static slatejs_ret_t %s_%s_%s(slatejs_context *ctx)\n",
...
/**
 * generate default values on the JavaScript stack
 */
static int
        ...
        int argidx)
{
        ...
        case WEBIDL_TYPE_STRING:
                ...
                outputf(outc,
                        "\t\tif (!slatejs_is_string(ctx, %d)) {\n"
                        "\t\t\tslatejs_to_string(ctx, %d);\n"
                        "\t\t}\n", argidx, argidx);
                break;

        case WEBIDL_TYPE_BOOL:
                outputf(outc,
			"\t\tif (!slatejs_is_boolean(ctx, %d)) {\n"
			"\t\t\tif (slatejs_is_number(ctx, %d)) {\n"
			"\t\t\t\tslatejs_to_boolean(ctx, %d);\n"
			"\t\t\t} else {\n"
			"\t\t\t\treturn slatejs_error(ctx, SLATEJS_ERR_ERROR, %s_error_fmt_bool_type, %d, \"%s\");\n"
			"\t\t\t}\n"
        ...
        case WEBIDL_TYPE_LONGLONG:
                outputf(outc,
                        "\t\tif (!slatejs_is_number(ctx, %d)) {\n"
                        "\t\t\treturn slatejs_error(ctx, SLATEJS_ERR_ERROR, %s_error_fmt_number_type, %d, \"%s\");\n"
                        "\t\t}\n",
                           ...
                           struct ir_operation_entry *operatione)
{
        ...
        outputf(outc,
                ...
                "\tslatejs_idx_t %s_argc = slatejs_get_top(ctx);\n\t", DLPFX);
        ...
        if (fixedargc > 0) {
                ...
                outputf(outc,
                        "\t\treturn slatejs_error(ctx, SLATEJS_RET_TYPE_ERROR, %s_error_fmt_argument, %d, %s_argc);\n",
        ...
        if (cdatac == 0) {
                ...
                if (options->dbglog) {
                        outputf(outc,
                              "\tNSLOG(quickjs, WARNING, \"Unimplemented\");\n");
                }
                        ...
                        struct ir_attribute_entry *atributee)
{
        /* getter definition */
        outputf(outc,
                "static slatejs_ret_t %s_%s_%s_getter(slatejs_context *ctx)\n",
                          ...
                          struct ir_attribute_entry *atributee)
{
        ...
        outputf(outc,
                "\tslatejs_put_prop_string(ctx, -2, \"%s\");\n\n",
                        ...
                        struct ir_attribute_entry *atributee)
{
       ...
       /* setter definition */
        outputf(outc,
                "static slatejs_ret_t %s_%s_%s_setter(slatejs_context *ctx)\n",
                DLPFX, interfacee->class_name, atributee->name);
        outputf(outc,
                "{\n");
...
/*
 * generate a source file to implement an interface using QuickJS and libdom.
 *
 * exported interface documented in quickjs-libdom.h
 */
