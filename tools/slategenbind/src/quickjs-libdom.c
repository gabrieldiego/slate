...
/**
 * \todo Constructors
 * \todo slatejs_bind_inject_not_ctr as binding.c function
...
output_makefile(struct ir *ir)
{
        ...
        fprintf(makef, "# quickjs libdom makefile fragment\n\n");

        fprintf(makef, "SLATEGENBIND_SOURCES:=binding.c ");
        for (idx = 0; idx < ir->entryc; idx++) {
                struct ir_entry *interfacee;
 ...
 *
 * The binding header contains all the quickjs-libdom specific binding interface
...
output_binding_header(struct ir *ir)
{
        ...
        outputf(bindc,
                "slatejs_bool_t %s_instanceof(slatejs_context *ctx, slatejs_idx_t index, const char *klass);\n",
                DLPFX);

        outputf(bindc,
                "slatejs_ret_t %s_create_prototypes(slatejs_context *ctx);\n", DLPFX);
 ...
 *
 * The binding header contains all the quickjs-libdom specific binding
 * implementations.
 */
static int
output_binding_src(struct ir *ir)
{
        ...
        outputf(bindc,
                "slatejs_bool_t\n"
                "%s_instanceof(slatejs_context *ctx, slatejs_idx_t _idx, const char *klass)\n"
                "{\n"
                "\tslatejs_idx_t idx = slatejs_normalize_index(ctx, _idx);\n"
                "\t/* ... ??? ... */\n"
                "\tif (!slatejs_check_type(ctx, idx, SLATEJS_TYPE_OBJECT)) {\n"
                "\t\treturn false;\n"
                "\t}\n"
                "\t/* ... obj ... */\n"
                "\tslatejs_get_global_string(ctx, %s_magic_string_prototypes);\n"
                "\t/* ... obj ... protos */\n"
                "\tslatejs_get_prop_string(ctx, -1, klass);\n"
                "\t/* ... obj ... protos goalproto */\n"
                "\tslatejs_get_prototype(ctx, idx);\n"
                "\t/* ... obj ... protos goalproto proto? */\n"
                "\twhile (!slatejs_is_undefined(ctx, -1)) {\n"
                "\t\tif (slatejs_strict_equals(ctx, -1, -2)) {\n"
                "\t\t\tslatejs_pop_3(ctx);\n"
                "\t\t\t/* ... obj ... */\n"
                "\t\t\treturn true;\n"
                "\t\t}\n"
                "\t\tslatejs_get_prototype(ctx, -1);\n"
        ...
        outputf(bindc,
                ...
                "%s_to_string(slatejs_context *ctx)\n"
                "{\n"
                "\t/* */\n"
                ...
                "\tslatejs_get_prototype(ctx, -1);\n"
                "\t/* this proto */\n"
                "\tslatejs_get_prop_string(ctx, -1, \"%sklass_name\");\n"
                "\t/* this proto classname */\n"
                "\tslatejs_push_string(ctx, \"[object \");\n"
        ...
        outputf(bindc,
                "static slatejs_ret_t %s_create_prototype(slatejs_context *ctx,\n",
                DLPFX);
        outputf(bindc,
                "\t\t\t\t\tslatejs_safe_call_function genproto,\n"
                "\t\t\t\t\tconst char *proto_name,\n"
                "\t\t\t\t\tconst char *klass_name)\n"
                "{\n"
                "\tslatejs_int_t ret;\n"
                "\tslatejs_push_object(ctx);\n"
                "\tif ((ret = slatejs_safe_call(ctx, genproto, NULL, 1, 1)) != SLATEJS_EXEC_SUCCESS) {\n"
                "\t\tslatejs_pop(ctx);\n"
                "\t\tNSLOG(quickjs, WARNING, \"Failed to register prototype for %%s\", proto_name + 2);\n"
                "\t\treturn ret;\n"
                "\t}\n"
                "\t/* top of stack is the ready prototype, inject it */\n"
                "\tslatejs_push_string(ctx, klass_name);\n"
                "\tslatejs_put_prop_string(ctx, -2, \"%sklass_name\");\n"
                "\tslatejs_push_c_function(ctx, %s_to_string, 0);\n"
                "\tslatejs_put_prop_string(ctx, -2, \"toString\");\n"
                "\tslatejs_push_string(ctx, \"toString\");\n"
                "\tslatejs_def_prop(ctx, -2, SLATEJS_DEFPROP_HAVE_ENUMERABLE);\n"
                "\tslatejs_put_global_string(ctx, proto_name);\n"
                "\treturn SLATEJS_ERR_NONE;\n"
                "}\n\n",
                MAGICPFX,
                DLPFX);
...
int slatejs_libdom_output(struct ir *ir)
{
        int idx;
        int res = 0;
