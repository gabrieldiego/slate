...
ssh "$ssh_target" "set -eu; ulimit -v $((remote_memory_mb * 1024)) 2>/dev/null || true; '$remote_dir/slate-broadwebd-net-probe' serve --bind 0.0.0.0:0 --state-root '$remote_dir/state' --ready-file '$remote_dir/ready' --max-requests 16 --frame-max-bytes '$frame_max_bytes' > '$remote_dir/server.log' 2>&1 & printf '%s\n' "'$!'" > '$remote_dir/server.pid'"
