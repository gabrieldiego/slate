...
#ifndef SLATE_SSL_CERTS_H_
#define SLATE_SSL_CERTS_H_
 ...
 * \return SLATEERROR_OK on success or SLATEERROR_NOMEM on memory exhaustion
 */
slateerror cert_chain_alloc(size_t depth, struct cert_chain **chain_out);
 ...
 * \return SLATEERROR_OK on success or SLATEERROR_NOMEM on memory exhaustion
 *
 * NOTE: if this returns SLATEERROR_NOMEM then the destination chain will have
 * some amount of content and should be cleaned up with cert_chain_free.
 */
slateerror cert_chain_dup_into(const struct cert_chain *src, struct cert_chain *dst);
 ...
 * \return SLATEERROR_OK on success or SLATEERROR_NOMEM on memory exhaustion
 */
slateerror cert_chain_dup(const struct cert_chain *src, struct cert_chain **dst_out);
 ...
 * \return SLATEERROR_OK on success or SLATEERROR_NOMEM on memory exhaustion
 */
slateerror cert_chain_from_query(struct slateurl *url, struct cert_chain **chain_out);
 ...
 * \return SLATEERROR_OK on success or SLATEERROR_NOMEM on memory exhaustion
 */
slateerror cert_chain_to_query(struct cert_chain *chain, struct slateurl **url_out);
 ...
 */
slateerror cert_chain_free(struct cert_chain *chain);
