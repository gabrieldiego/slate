...
#ifndef SLATE_BROWSER_WINDOW_H_
#define SLATE_BROWSER_WINDOW_H_
 ...
 */
slateerror browser_window_create(enum browser_window_create_flags flags,
		struct slateurl *url, struct slateurl *referrer,
 ...
 *
 */
slateerror browser_window_navigate(struct browser_window *bw,
			     struct slateurl *url,
			     struct slateurl *referrer,
 ...
 *
 * \param bw browser window
 * \return pointer to slateurl. Doesn't create a ref for caller.
 *
 * \note guaranteed to return a valid slateurl ptr, never returns NULL.
 */
struct slateurl* browser_window_access_url(const struct browser_window *bw);
 ...
 * \return SLATEERROR_OK, or appropriate error otherwise.
 */
slateerror browser_window_get_url(
		struct browser_window *bw,
		bool fragment,
 ...
 * \return SLATEERROR_OK and scale applied else other error code caused by reflow etc.
 */
slateerror browser_window_set_scale(struct browser_window *bw, float scale, bool absolute);
 ...
 */
slateerror browser_window_get_features(struct browser_window *bw,
 ...
 * \return SLATEERROR_OK, or appropriate error otherwise
 */
slateerror browser_window_get_scrollbar_type(struct browser_window *bw,
 ...
 */
slateerror browser_window_debug_dump(struct browser_window *bw, FILE *f, enum content_debug op);
 ...
 */
slateerror browser_window_get_name(struct browser_window *bw, const char **name);
 ...
 * \return SLATEERROR_OK and the name is updated or SLATEERROR_NOMEM and the
 *         original name is untouched.
 */
slateerror browser_window_set_name(struct browser_window *bw, const char *name);
 ...
 */
slateerror browser_window_console_log(struct browser_window *bw,
 ...
 *
 * If there is no chain available, this will return SLATEERROR_NOT_FOUND
 *
 * \param bw The browser window
 * \param chain Pointer to be filled out with certificate chain
 * \return Whether or not the chain is available
 */
slateerror browser_window_get_ssl_chain(struct browser_window *bw, struct cert_chain **chain);
 ...
 * \return SLATEERROR_OK, or appropriate error otherwise.
 */
slateerror browser_window_show_certificates(
		struct browser_window *bw);
