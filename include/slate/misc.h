...
#ifndef SLATE_MISC_H_
#define SLATE_MISC_H_
...
struct gui_misc_table {
	 ...
	 *
	 */
	slateerror (*schedule)(int t, void (*callback)(void *p), void *p);
	 ...
	 */
	slateerror (*launch_url)(struct slateurl *url);
	 ...
	 * is required.
	 *
	 * To keep hold of the url, remember to slateurl_ref() it, and to keep
	 * the realm, you will need to strdup() it.
	 *
	 * If the front end returns SLATEERROR_OK for this function, they may,
	 * at some future time, call the `cb` with `cbpw` callback exactly once.
	 *
	 * If the front end returns other than SLATEERROR_OK, they should not
	 ...
	 * \return SLATEERROR_OK on sucess else error and cb never called
	 */
	slateerror (*login)(struct slateurl *url, const char *realm,
	 ...
	 */
	slateerror (*present_cookies)(const char *search_term);
};
